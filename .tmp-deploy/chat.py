...
async def _check_topic_changed(prev_context: str, curr_context: str, cfg) -> bool:
    """Ask LLM whether the conversation topic has shifted significantly."""
    if not prev_context.strip():
        return True

    prompt = (
        "Compare the conversation topics. Has the topic shifted significantly?\n\n"
        f"Previous:\n{prev_context[:800]}\n\n"
        f"Current:\n{curr_context[:800]}\n\n"
    ...
    try:
        ...
        if not base_url or not api_key:
            return True  # if no LLM available, always refresh (safe default)
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            logger.debug("Topic check: prev_chars=%d curr_chars=%d changed=%s", len(prev_context), len(curr_context), changed)
            return changed
    except Exception as exc:
        logger.warning("Topic check failed (degraded, will refresh): %s", exc)
        return True  # on failure, refresh to be safe
...
async def _precompute_context_embedding(cfg, services, ctx, original_messages: list[dict],
    ...
    if prev_text and last_turn is not None:
        ...
        if turns_since >= _CONTEXT_REFRESH_INTERVAL:
            logger.debug("Context refresh forced: conv=%s turns_since=%d", ctx.conversation_id, turns_since)
        else:
            topic_changed = await _check_topic_changed(prev_text, context_text, cfg)
            if not topic_changed:
                should_refresh = False
                logger.debug("Context refresh skipped: conv=%s topic unchanged turns_since=%d", ctx.conversation_id, turns_since)
    ...
    if should_refresh:
        ...
        logger.debug("Precomputed context embedding for conv=%s chars=%d turn=%d", ctx.conversation_id, len(context_text), turn)
