...
# 模型别名映射: RikkaHub 识别的模型名 → 实际上游模型名
# (当前所有模型名已直接匹配 LiteLLM 路由，无需映射)
_MODEL_ALIASES = {}
...
class ChatPipeline:
    ...
    async def inject_memory(self, prepared: ChatPipelineContext) -> None:
        ...
        if not (
            ...
            return
        # Skip proxy injection for Hermes Agent requests.
        # Hermes has its own KokoroMemo memory provider plugin
        # (direct SQLite read), so proxy-layer injection would
        # cause memory to be injected twice.
        # Detection: Hermes sends x-agent: hermes header.
        if prepared.request.headers.get("x-agent") == "hermes":
            return
        try:
            ctx = prepared.ctx
